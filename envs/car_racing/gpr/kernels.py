...
class ExpSineSquared(Kernel,sk_ExpSineSquared):
 def gradient_x(self,x,X_train):
  ...
  return np.expand_dims(grad_wrt_theta*exp_sine_squared*grad_wrt_exp,axis=1)*diff
...
class Product(Kernel,sk_Product):
 def gradient_x(self,x,X_train):
  ...
  X_train=np.asarray(X_train)
  f_ggrad=(np.expand_dims(self.k1(x,X_train)[0],axis=1)*self.k2.gradient_x(x,X_train))
  fgrad_g=(np.expand_dims(self.k2(x,X_train)[0],axis=1)*self.k1.gradient_x(x,X_train))
...
class HammingKernel(sk_StationaryKernelMixin,sk_NormalizedKernelMixin,Kernel):
 ...
 def hyperparameter_length_scale(self):
  ...
  if anisotropic:
   return Hyperparameter("length_scale","numeric",self.length_scale_bounds,len(length_scale))
 ...
 def __call__(self,X,Y=None,eval_gradient=False):
  ...
  if anisotropic and X.shape[1]!=len(length_scale):
   raise ValueError("Expected X to have %d features, got %d"%(len(length_scale),X.shape[1]))
