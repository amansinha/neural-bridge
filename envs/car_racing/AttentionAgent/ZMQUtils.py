...
def Args_getSockets(args):
 ...
 for i in range(num_workers):
  ...
  os.system('xvfb-run -a -s "-screen 0 1400x900x24 +extension RANDR" python3 worker.py --log-dir=pretrained/CarRacing/'+' --source_worker_port '+str(source_worker_port)+' --worker_sink_port '+str(worker_sink_port)+' --source_worker_direct_port '+str(source_worker_direct_port+i)+' 1>&2 &')
  time.sleep(0.5)
